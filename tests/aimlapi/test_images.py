...
@pytest.mark.respx(base_url=AIML_BASE_URL)
def test_image_stream_gzip_body_is_decoded(aiml_client, respx_mock: MockRouter) -> None:
    payload = {
    ...
    assert response.data[0].b64_json == base64.b64encode(image_bytes).decode()
    assert response.data[0].url == "https://cdn.example/image.png"


@pytest.mark.asyncio
@pytest.mark.respx(base_url=AIML_BASE_URL)
async def test_async_image_stream_gzip_body_is_decoded(respx_mock: MockRouter) -> None:
    payload = {
        "data": [{"b64_json": None, "url": "https://cdn.example/image.png"}],
    }
    gzipped = gzip.compress(json.dumps(payload).encode("utf-8"))
    image_bytes = b"async-stream-bytes"
    ...
    try:
        async with client.images.generate(prompt="a lighthouse", response_format="b64_json", stream=True) as stream:
...
@pytest.mark.respx(base_url=AIML_BASE_URL)
def test_image_stream_events_enriched(aiml_client, respx_mock: MockRouter) -> None:
    image_bytes = b"stream-event-bytes"
    event_payload = {
    ...
    stream_body = f"data: {json.dumps(event_payload)}\n\n".encode()
    ...
    stream = aiml_client.images.generate(
        prompt="a lighthouse",
    ...
    assert event.b64_json == base64.b64encode(image_bytes).decode()
    assert getattr(event, "url", None) == "https://cdn.example/image.png"
